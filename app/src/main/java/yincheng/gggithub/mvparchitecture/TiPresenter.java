...
import yincheng.gggithub.mvparchitecture.internal.OneTimeRemovable;
import yincheng.gggithub.mvparchitecture.internal.UiThreadExecutor;
import yincheng.gggithub.mvparchitecture.test.TiTestPresenter;
...
public abstract class TiPresenter<V extends TiView> {
    ...
    */
   private boolean mCalled = true;//生命周期函数禁止外部调用
    ...
    * 如果队列是空的，消费者会一直等待，当生产者添加元素时候，消费者是如何知道当前队列有元素的呢？这里使用了使用观察者
    * 模式实现。就是当生产者往满的队列里添加元素时会阻塞住生产者，当消费者消费了一个队列中的元素后，会通知生产者当前队
    * 列可用。
    * 这里使用了{@link Condition}来实现
    */
   ...
   /**
    * 只能通过{@link #detachView()} 来设置绑定的View为null
    */
   public void attachView(@NonNull final V view) {
      ...
      mView = view;
      //不一定是只有调用了生命周期函数以后才能执行observer.onChange()方法，这里也可以，因为onChange()方法
      //两个参数，第二个就是hasLifecycleMethodBeenCalled
      moveToState(State.VIEW_ATTACHED, false);
      mCalled = false;
      TiLog.v(TAG, "onAttachView(TiView)");
      onAttachView(view);
      if (!mCalled) {//保证生命周期函数被调用了
         throw new SuperNotCalledException("Presenter " + this
               + " did not call through to super.onAttachView(TiView)");
      }
      //这里一定为true，因为前面用mCalled进行了判断，只要走到这一句就一定调用了生命周期函数，否则抛出异常
    ...
    * @see #onDestroy()
    */
   protected void onCreate() {//protected，禁止外部调用生命周期函数
      if (mCalled) {
   ...
   /**
    * moves the presenter to the new state and validates(验证) the correctness(正确性) of the
    * transition(过渡)
    *
    * @param newState                     the new state to set
    * @param hasLifecycleMethodBeenCalled 是否在调用moveToState方法之前调用过presenter的生命周期函数
    *                                     {@link #onCreate(),#onAttachView(TiView),#onDetachView,#onDestroy}
    */
...
//状态演变：INITIALIZED -> VIEW_DETACHED -> {VIEW_ATTACHED -> VIEW_DETACHED}/DESTROYED
      ...
      if (mLifecycleObservers.size() > 0) {
         ...
         switch (newState) {
            ...
            case DESTROYED:
               ...
               for (int i = observers.size() - 1; i >= 0; i--) {// TODO: 2018/5/23  
                  observers.get(i).onChange(newState, hasLifecycleMethodBeenCalled);
               }
         }
      }
   ...
   /**
    * Executes all postponed(推迟的) view actions
    *
