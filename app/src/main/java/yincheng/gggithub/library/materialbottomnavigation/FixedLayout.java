...
import static yincheng.gggithub.library.materialbottomnavigation.MiscUtils.log;
...
public class FixedLayout extends ViewGroup implements ItemsLayoutContainer {
   ...
   public void setSelectedIndex(final int index, final boolean animate) {
      log(TAG, Log.INFO, "setSelectedIndex: " + index);
      ...
      final BottomNavigationFixedItemView current = (BottomNavigationFixedItemView) getChildAt
            (oldSelectedIndex);
   ...
   public void setItemEnabled(final int index, final boolean enabled) {
      ...
      final BottomNavigationItemViewAbstract child = (BottomNavigationItemViewAbstract)
            getChildAt(index);
      if (null != child) {
   ...
   public void populate(@NonNull final MenuParser.Menu menu) {
      log(TAG, Log.INFO, "populate: " + menu);
   ...
   private void populateInternal(@NonNull final MenuParser.Menu menu) {
      ...
      int proposedWidth = Math.min(Math.max(screenWidth / menu.getItemsCount(),
            minActiveItemWidth), maxActiveItemWidth);
      ...
      for (int i = 0; i < menu.getItemsCount(); i++) {
         ...
         LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(proposedWidth,
               getHeight());
