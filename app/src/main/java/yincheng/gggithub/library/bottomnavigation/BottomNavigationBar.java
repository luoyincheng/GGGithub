package yincheng.gggithub.library.bottomnavigation;
...
import yincheng.gggithub.library.bottomnavigation.behaviour.BottomNavBarFabBehaviour;
import yincheng.gggithub.library.bottomnavigation.behaviour.BottomVerticalScrollBehavior;
import yincheng.gggithub.library.bottomnavigation.utils.Utils;
...
public class BottomNavigationBar extends FrameLayout {
   ...
   @Retention(RetentionPolicy.SOURCE) @interface BackgroundStyle {
   }
   ...
   @IntDef({FAB_BEHAVIOUR_TRANSLATE_AND_STICK, FAB_BEHAVIOUR_DISAPPEAR,
         FAB_BEHAVIOUR_TRANSLATE_OUT})
   @Retention(RetentionPolicy.SOURCE) @interface FabBehaviour {
   }
   ...
   public BottomNavigationBar(Context context, AttributeSet attrs, int defStyleAttr, int
         defStyleRes) {
   ...
   private void parseAttrs(Context context, AttributeSet attrs) {
      if (attrs != null) {
         TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable
               .BottomNavigationBar, 0, 0);

         mActiveColor = typedArray.getColor(R.styleable.BottomNavigationBar_bnbActiveColor, Utils
               .fetchContextColor(context, R.attr.colorAccent));
         mInActiveColor = typedArray.getColor(R.styleable.BottomNavigationBar_bnbInactiveColor,
               Color.LTGRAY);
         mBackgroundColor = typedArray.getColor(R.styleable
               .BottomNavigationBar_bnbBackgroundColor, Color.WHITE);
         mAutoHideEnabled = typedArray.getBoolean(R.styleable
               .BottomNavigationBar_bnbAutoHideEnabled, true);
         mElevation = typedArray.getDimension(R.styleable.BottomNavigationBar_bnbElevation,
               getResources().getDimension(R.dimen.bottom_navigation_elevation));

         setAnimationDuration(typedArray.getInt(R.styleable
               .BottomNavigationBar_bnbAnimationDuration, DEFAULT_ANIMATION_DURATION));
         ...
         switch (typedArray.getInt(R.styleable.BottomNavigationBar_bnbBackgroundStyle,
...
//    }
   ...
   public void initialise() {
      ...
      if (!mBottomNavigationItems.isEmpty()) {
         ...
         if (mMode == MODE_FIXED || mMode == MODE_FIXED_NO_TITLE) {
            ...
            int[] widths = BottomNavigationHelper.getMeasurementsForFixedMode(getContext(),
                  screenWidth, mBottomNavigationItems.size(), mScrollable);
            int itemWidth = widths[0];

            for (BottomNavigationItem currentItem : mBottomNavigationItems) {
               FixedBottomNavigationTab bottomNavigationTab = new FixedBottomNavigationTab
                     (getContext());
               setUpTab(mMode == MODE_FIXED_NO_TITLE, bottomNavigationTab, currentItem,
                     itemWidth, itemWidth);
            }

         } else if (mMode == MODE_SHIFTING || mMode == MODE_SHIFTING_NO_TITLE) {

            int[] widths = BottomNavigationHelper.getMeasurementsForShiftingMode(getContext(),
                  screenWidth, mBottomNavigationItems.size(), mScrollable);
            ...
            for (BottomNavigationItem currentItem : mBottomNavigationItems) {
               ShiftingBottomNavigationTab bottomNavigationTab = new ShiftingBottomNavigationTab
                     (getContext());
               setUpTab(mMode == MODE_SHIFTING_NO_TITLE, bottomNavigationTab, currentItem,
                     itemWidth, itemActiveWidth);
            }
         }
    ...
    * @return this, to allow builder pattern
    */
   public BottomNavigationBar setTabSelectedListener(OnTabSelectedListener
                                                           tabSelectedListener) {
    ...
    * @param forcedSelection if bottom navigation bar forced to select tab (in this case call on
    *                        selected irrespective of previous state
    */
         ...
         if (mBackgroundStyle == BACKGROUND_STYLE_STATIC) {
            ...
            if (firstTab) {
               ...
               mBackgroundOverlay.post(new Runnable() {
                  ...
                  public void run() {
//                            try {
                     BottomNavigationHelper.setBackgroundWithRipple(clickedView, mContainer,
                           mBackgroundOverlay, clickedView.getActiveColor(),
...
//                && oldPosition != -1) {
   ...
   public void setFab(FloatingActionButton fab) {
      ...
      if (layoutParams != null && layoutParams instanceof CoordinatorLayout.LayoutParams) {
         CoordinatorLayout.LayoutParams coLayoutParams = (CoordinatorLayout.LayoutParams)
               layoutParams;
   ...
   /**
    * Simple implementation of the OnTabSelectedListener interface with stub implementations of
    * each method.
