...
/**
 * Created by yincheng on 2018/6/4/15:35.
 * github:luoyincheng
 */
public class AnimHelper {
   private static final Interpolator FAST_OUT_LINEAR_IN_INTERPOLATOR = new
         FastOutLinearInInterpolator();
   private static final Interpolator LINEAR_OUT_SLOW_IN_INTERPOLATOR = new
         LinearOutSlowInInterpolator();
   private static final Interpolator interpolator = new LinearInterpolator();

   @UiThread private static void animateVisibility(@Nullable final View view, final boolean show,
                                                   int visibility) {
      if (view == null) {
         return;
      }
   ...
   @UiThread private static void animateSafeVisibility(final boolean show, @NonNull final View
         view, int visibility) {
      view.animate().cancel();
      ViewPropertyAnimator animator = view.animate().setDuration(200).alpha(show ? 1F : 0F)
   ...
   @UiThread public static void revealPopupWindow(@NonNull PopupWindow popupWindow, @NonNull View
         from) {
         ...
         if (view != null) {
            ...
            view.post(() -> {
               if (ViewCompat.isAttachedToWindow(view)) {
                  ...
                  animator.setDuration(view.getResources().getInteger(android.R.integer
         ...
         if (view != null) {
            view.post(() -> {
               if (ViewCompat.isAttachedToWindow(view)) {
                  ...
                  Animator animator = ViewAnimationUtils.createCircularReveal(view, centerX,
                        centerY, 20, view.getHeight());
   ...
   @UiThread public static void dismissDialog(@NonNull DialogFragment dialogFragment, int
         duration, AnimatorListenerAdapter listenerAdapter) {
      ...
      if (dialog != null) {
         if (dialog.getWindow() != null) {
            ...
            if (view != null) {
               int centerX = view.getWidth() / 2;
               int centerY = view.getHeight() / 2;
               float radius = (float) Math.sqrt(view.getWidth() * view.getWidth() / 4 + view
                     .getHeight() * view.getHeight() / 4);
               view.post(() -> {
                  if (ViewCompat.isAttachedToWindow(view)) {
                     Animator animator = ViewAnimationUtils.createCircularReveal(view, centerX,
                           centerY, radius, 0);
